    ...
    public class OpenXmlMemoryStreamDocument : IDisposable
    {
        ...
        public static OpenXmlMemoryStreamDocument CreateSpreadsheetDocument()
        {
            ...
            using (SpreadsheetDocument doc = SpreadsheetDocument.Create(stream, DocumentFormat.OpenXml.SpreadsheetDocumentType.Workbook))
            {
                ...
                // Start of fix proposed by Thomas Barnekow
                // Define our own relationship ID.
                string id = "rId1";

                // Create a WorkbookPart with a workbook root element containing
                // one sheet reference. A sheet reference is required for a 
                // "minimum workbook" as defined by the Standard ECMA-376.
                    ...
                    new XElement(ns + "workbook",
                        ...
                        new XElement(ns + "sheets",
                            new XElement(ns + "sheet",
                                ...
                                new XAttribute(relationshipsns + "id", id))))));

                // We also have to create a WorksheetPart with basic worksheet root element
                // containing an empty sheetData child. Just the WorkbookPart with a workbook
                // root element that does not contain any sheet reference does not work.
                // A document like that can't be opened with Excel. 
                WorksheetPart worksheetPart = doc.WorkbookPart.AddNewPart<WorksheetPart>(id);
