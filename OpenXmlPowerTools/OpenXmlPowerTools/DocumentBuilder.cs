                ...
                if (idElement != null)
                {
                    ...
                    if (numId != "0")
                    {
                        ...
                        XElement newElement = newNumbering
                                ...
                                .Elements(W.num)
                                .Where(e => e.Annotation<FromPreviousSourceSemaphore>() == null)
                                .Where(e => e.Annotation<string>() == numId)    // Change by Thomas Barnekow. See related change below.
                        ...
                        if (newElement == null)
                        {
                            ...
                            newElement
                                ...
                                .Attribute(W.val).Value = newAbstractId;
                            newElement.Attribute(W.numId).Value = number.ToString();
                            // Start of change by Thomas Barnekow: Adding w:numId
                            // attribute value as annotation. This will make
                            // DocumentBuilder create a new w:num element for each 
                            // original w:num element.
                            // This way, we support the restarting of a numbered
                            // list, which requires one additional w:num element
                            // each time numbering is restarted.
                            newElement.AddAnnotation(numId);
                            // End of change by Thomas Barnekow
                            number++;
                            newNumbering.Root.Add(newElement);
                        }
                        idElement.Attribute(W.val).Value = newElement.Attribute(W.numId).Value;
